...
public class LexAnalyser {
	...
	public void addIdentifier(VarToken t) {
	...
	public int readInt() throws IOException {
		if (Character.isDigit(peek)) {
			...
			System.out.println("Error charactar is not a digit");
			return 0;
		}
	}
	...
	public int scan() throws IOException {
    	...
    	do {
	    	...
	    	// Otherwise, check if the next token is a valid identifier or else a keyword
	            ...
	            else if(nextWord.equals("BOOL")) {
	            	nextToken = new Token(TokenType.BOOL_KEYWORD);
	            	addTokenToArray(nextToken);
	            	// Since this is an INT keyword, the next token must be an integer variable
	            	removeWhitespace();
	            	nextWord = readString();
	            	nextToken = new BoolVarToken(nextWord);
	            	addTokenToArray(nextToken);
	            	addIdentifier((BoolVarToken) nextToken);
	            	continue;
	            } if(nextWord.equals("true")||nextWord.equals("false")) { 
	    			String readValue = readString();
	    			nextToken = new BoolToken(Boolean.parseBoolean(readValue), TokenType.BOOLVAL);
	    			addTokenToArray(nextToken);
	   				continue; // Restart the loop to find next token
		    	} 
			...
			case ('!'):
				peek = ' ';
				nextToken = new Token(TokenType.NOT);
				addTokenToArray(nextToken);
				break;
			case (';'):
				peek = ' ';
			...
			case ('|'):
				peek = ' ';
				nextToken = new Token(TokenType.OR);
				addTokenToArray(nextToken);
				break;
			case ('&'):
				peek = ' ';
				nextToken = new Token(TokenType.AND);
				addTokenToArray(nextToken);
				break;
			case (':'):
				peek = ' ';
